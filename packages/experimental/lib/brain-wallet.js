...
var BrainWallet = /** @class */ (function (_super) {
    ...
    BrainWallet._generate = function (username, password, legacy, progressCallback) {
        ...
        if (typeof (username) === 'string') {
            ...
            usernameBytes = swisstronik_ethers_1.ethers.utils.toUtf8Bytes(username.normalize('NFKC'));
        }
        else {
            usernameBytes = swisstronik_ethers_1.ethers.utils.arrayify(username);
        }
        if (typeof (password) === 'string') {
            logger.checkNormalize();
            passwordBytes = swisstronik_ethers_1.ethers.utils.toUtf8Bytes(password.normalize('NFKC'));
        }
        else {
            passwordBytes = swisstronik_ethers_1.ethers.utils.arrayify(password);
        }
        return scrypt_js_1.default.scrypt(passwordBytes, usernameBytes, (1 << 18), 8, 1, 32, progressCallback).then(function (key) {
            ...
            var mnemonic = swisstronik_ethers_1.ethers.utils.entropyToMnemonic(swisstronik_ethers_1.ethers.utils.arrayify(key).slice(0, 16));
            return new BrainWallet(swisstronik_ethers_1.ethers.Wallet.fromMnemonic(mnemonic));
        });
    };
    ...
    return BrainWallet;
}(swisstronik_ethers_1.ethers.Wallet));
